...
if ($has_cart || $header_search || $action_link || $wpml_icon) {
	...
	echo '<div class="top_bar_right">';
		...
		echo '<div class="top_bar_right_wrapper">';
			...
			if ( is_active_sidebar( 'kaman-stock-amount' ) ) : 

				dynamic_sidebar( 'kaman-stock-amount' );
			...
			if ($header_search == 'input') {
				...
				echo '<a aria-label="Search" id="search_button" class="has-input">';
					...
					echo '<form method="get" id="searchform" action="'. esc_url(home_url('/')) .'">';
						...
						echo '<label for="s" class="visually-hidden">Search</label>';
						echo '<input aria-label="Search" type="text" class="field" name="s" placeholder="'. esc_html($translate['search-placeholder']) .'" />';
						...
						echo '<input aria-label="Submit" type="submit" class="submit" value="" style="display:none;" />';
				...
				echo '<a id="search_button" href="#"><span class="visually-hidden">Search</span><i class="icon-search-fine"></i></a>';
