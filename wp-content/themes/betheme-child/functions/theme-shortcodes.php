			...
			$output .= '<ul class="ul-first">';
				...
				while ($query_blog->have_posts()) {
					...
					$output .= '<li class="'. esc_attr(implode(' ', get_post_class())) .'">';
						...
						$output .= '<div class="photo">';
							$output .= '<a aria-label="'. get_the_title() .'" href="'. esc_url(get_permalink()) .'">';
								$output .= get_the_post_thumbnail(get_the_ID(), 'blog-portfolio', array( 'class' => 'scale-with-grid' ));
							$output .= '</a>';
						$output .= '</div>';
						...
						$output .= '<div class="desc">';
							...
							if ($excerpt == '1' || ($first && $excerpt == 'featured')) {
								$output .= '<div class="post-excerpt">'. get_the_excerpt(get_the_ID()) .'</div><div style="display:none;">' .get_the_ID(). '</div>';
							}
					...
					if ($first) {
						if ($link) {
							$output .= '<a class="linked-btn featured-news-link" href="'. esc_url($link) .'"></span><span class="button_label">'. esc_html($link_title) .'</span></a>';
						}
...
if (! function_exists('sc_hover_box')) {
	function sc_hover_box($attr, $content = null)
	{
		...
		// output -----
		$btn_link_text = ( ( !empty( $link_text ) ) ? $link_text : 'View Products' );
		...
		if( !empty( $link_type ) ) {
			$output = '<div class="hover_box">';
				...
				$output .= '<div class="hover_box_wrapper '. $draft .'">';
					$output .= '<img class="visible_photo scale-with-grid" src="'. esc_url($image) .'" alt="'.esc_html($product_heading).'" width="'. esc_attr(mfn_get_attachment_data($image, 'width')) .'" height="'. esc_attr(mfn_get_attachment_data($image, 'height')) .'"/>';
					$output .= '<img class="hidden_photo scale-with-grid" src="'. esc_url($image_hover) .'" alt="" width="'. esc_attr(mfn_get_attachment_data($image_hover, 'width')) .'" height="'. esc_attr(mfn_get_attachment_data($image_hover, 'height')) .'"/>';
					$output .= '<div class="product-section-heading"><h3>'.$product_heading.'</h3></div>';
					$output .= '<div class="product-section-description"><h3>'.$product_heading.'</h3><p>'.$product_short_description.'</p>';

					if ($link != '') {
						$output .= '<span class="a-tag-link full-box-link">'.$btn_link_text.'  </span>';
					}
					$output .= '</div>';
				$output .= '</div>';
				if ($link) {
...
if (! function_exists('certifications_and_accreditation_callback')) {
	function certifications_and_accreditation_callback($attr, $content = null)
	{
		extract(shortcode_atts(array(
			'c_and_a_title'	=> '',
			'c_and_a_sub_title' 	=> '',
		...
		ob_start();
		?>
		<div class="certifications-and-accreditation">
			<a class="linked-btn" href="<?php echo $c_and_a_link; ?>">
				<h3><?php echo $c_and_a_title; ?></h3>
				<p><?php echo $c_and_a_sub_title; ?></p>
				<span>></span>
			</a>
		</div>
		<?php
...
if (! function_exists('mfn_shortcodes')) {
	function mfn_shortcodes(){
		...
		add_shortcode('certifications_and_accreditation', 'certifications_and_accreditation_callback');
