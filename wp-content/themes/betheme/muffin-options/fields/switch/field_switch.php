			...
			if( !$vb ) echo '<div class="form-control">';
				...
				echo '<ul class="'. esc_attr( $preview ) .'">';
					...
					foreach ( $this->field['options'] as $k => $v ) {
						...
						if( $js ){
							if( isset($this->field['version']) && $this->field['version'] == 'multiple' ){
								echo '<li class="\'+('.$js.' && '.$js.'.includes("'. esc_attr($k) .'") ? "active" : "") +\'" '. $tooltip .' '. $tooltipActive .'>';
							}else{
								echo '<li class="\'+( ('.$js.' && '.$js.' == "'. esc_attr($k) .'") || ( !'.$js.' && "'. esc_attr($k) .'" == "0" ) || ( !'.$js.' && !"'. esc_attr($k) .'" ) ? "active" : "") +\'" '. $tooltip .' '. $tooltipActive .'>';
							}
						}else{
						echo '<li class="'. $class .'" '. $tooltip .' '. $tooltipActive .'>';
							...
							echo '<fieldset>';
								if( $js ){
									if( isset($this->field['version']) && $this->field['version'] == 'multiple' ){
										echo '<input type="checkbox" \'+('.$js.' && '.$js.'.includes("'. esc_attr($k) .'") ? "checked" : "") +\' value="'. esc_attr( $k ) .'" '. $this->get_name( $meta ) .' autocomplete="off" />';
									}else{
										echo '<input type="checkbox" \'+( ('.$js.' && '.$js.' == "'. esc_attr($k) .'") || ( !'.$js.' && "'. esc_attr($k) .'" == "0" ) || ( !'.$js.' && !"'. esc_attr($k) .'" ) ? "checked" : "") +\' value="'. esc_attr( $k ) .'" '. $this->get_name( $meta ) .' autocomplete="off" />';
									}
								}else{
