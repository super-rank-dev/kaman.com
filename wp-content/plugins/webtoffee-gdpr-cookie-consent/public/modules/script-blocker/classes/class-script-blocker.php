			...
			function( $matches ) use ( $autoData, $elm_position ) {
				...
				if ( strpos( $match, 'data-cli-class' ) === false ) {
					...
					if ( ( preg_match( '/<iframe[^>]+?(data-src\s*=\s*(?:"|\')(.*)(?:"|\')).*>.*<\/iframe>/i', $match, $element_match ) )
						|| ( preg_match( '/<iframe[^>]+?(src\s*=\s*(?:"|\')(.*)(?:"|\')).*>.*<\/iframe>/i', $match, $element_match ) )
						...
						if ( preg_match( '/<script(?s).*?(type=(?:"|\')(.*?)(?:"|\')).*?>/', $match ) && preg_match( '/<script(?s).*?(type=(?:"|\')text\/javascript(.*?)(?:"|\')).*?>/', $match ) ) {
							preg_match( '/<script(?s).*?(type=(?:"|\')text\/javascript(.*?)(?:"|\')).*?>/', $match, $output_array );
