...
class Cookie_Law_Info_Shortcode {
	...
	*/
		...
		*/
		...
		foreach ( $cookie_list as $key => $cookie ) {
			...
			if ( count( $cookies ) > 0 ) {
				...
				$ret .= '<tr class="cookielawinfo-row-cat-title"><th colspan="' . count( $columns ) . '" class="cookielawinfo-row-cat-title-head">' . $cookie['title'] . '</th></tr>';
	...
	public function render_cookie_raw_table( $ret, $columns, $cookie_post = array() ) {
		...
		$ret .= '<tr class="cookielawinfo-row">';
		if ( in_array( 'cookie', $columns ) ) {
			$ret .= '<td class="cookielawinfo-column-1">' . $cookie_post['title'] . '</td>';
		}
		if ( in_array( 'type', $columns ) ) {
			$ret .= '<td class="cookielawinfo-column-2">' . $cookie_post['type'] . '</td>';
		}
		if ( in_array( 'duration', $columns ) ) {
			$ret .= '<td class="cookielawinfo-column-3">' . $cookie_post['duration'] . '</td>';
		}
		if ( in_array( 'description', $columns ) ) {
			$ret .= '<td class="cookielawinfo-column-4">' . $cookie_post['description'] . '</td>';
		}
		$ret .= '</tr>';
		return $ret;
	}
	...
	public function cookielawinfo_shortcode_reject_button( $atts ) {
		...
		$url_reject = ( $settings['button_3_action'] == 'CONSTANT_OPEN_URL' && $settings['button_3_url'] != '#' ) ? "href='$settings[button_3_url]'" : "role='button'";
		$link_tag   = '';
		$link_tag  .= '<a id="wt-cli-reject-btn" tabindex="0" ' . $url_reject . ' style="' . $styles . '" ';
	...
	public function cookielawinfo_shortcode_settings_button( $atts ) {
		...
		$url_s     = ( $settings['button_4_action'] == 'CONSTANT_OPEN_URL' && $settings['button_4_url'] != '#' ) ? "href='$settings[button_4_url]'" : "role='button'";
		$link_tag  = '';
		$link_tag .= '<a id="wt-cli-settings-btn" tabindex="0" ' . $url_s . ' style="' . $styles . '"';
	...
	public function cookielawinfo_shortcode_main_button( $atts ) {
		...
		$url = ( $settings['button_1_action'] == 'CONSTANT_OPEN_URL' && $settings['button_1_url'] != '#' ) ? "href='$settings[button_1_url]'" : "role='button'";

		// adding custom style
		$styles    = $this->generateStyle( $settings, 'button_1_style' );
		$link_tag  = '<a id="wt-cli-accept-btn" tabindex="0" ' . $url . ' style="' . $styles . '" data-cli_action="accept" ';
	...
	public function cookielawinfo_accept_all_button() {
		...
		$link_tag  = '<a id="wt-cli-accept-all-btn" tabindex="0" ' . $url . ' style="' . $styles . '" data-cli_action="accept_all" ';
	...
	public function cookielawinfo_shortcode_button_DRY_code( $name ) {
		...
		// If is action not URL then don't use URL!
		$url       = ( $settings['button_x_action'] == 'CONSTANT_OPEN_URL' && $settings['button_x_url'] != '#' ) ? "href='$settings[button_x_url]'" : "role='button'";
		$link_tag  = '<a id="wt-cli-policy-link" tabindex="0" ' . $url . '' . $rel . ' style="' . $styles . '"';
