...
class Elegant_Tabs_VC_Envato_API {
	...
	public function request( $url, $args = array() ) {
		...
		if ( empty( $this->token ) ) {
			return new WP_Error( 'api_token_error', __( 'An API token is required.', 'elegant-tabs' ) );
		}
		...
		if ( 200 !== $response_code ) {
			return new WP_Error( $response_code, __( 'An unknown API error occurred.', 'elegant-tabs' ) );
		}
		$return = json_decode( wp_remote_retrieve_body( $response ), true );
		if ( null === $return ) {
			return new WP_Error( 'api_error', __( 'An unknown API error occurred.', 'elegant-tabs' ) );
		}
		return $return;
	}
