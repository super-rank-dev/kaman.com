...
class WsForms extends Base {
	...
	public function register_plugin( $forms ) {
		$forms[ $this->plugin_name ] = 'WS Forms';
		return $forms;
	}
	...
	private function prepare_posted_data( $fields, $form ) {
		$posted_data = [];
		$uploads_dir = trailingslashit( wp_upload_dir()['basedir'] ) . 'form-vibes/ws-forms';
		if ( ! file_exists( $uploads_dir ) ) {
			wp_mkdir_p( $uploads_dir );
		}

		$wsupload       = wp_upload_dir();
		$fv_dirname     = $wsupload['baseurl'] . '/form-vibes/ws-forms';
		$uploaded_files = [];

		foreach ( $fields as $key => $values ) {
			if ( $key === 'wsf_meta_key_hidden' ) {
				continue;
			}
			if ( $values['type'] === 'file' ) {
				if($values['value']){
		...
		// echo '<pre>'; print_r($posted_data); echo '</pre>'; //die();
		return $posted_data;
	}
